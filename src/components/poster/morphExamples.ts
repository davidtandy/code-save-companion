...
export function buildMorphInfo(possessiveId: string, einArticleId: string): MorphInfo | null {
  const stem = POSSESSIVE_STEM[possessiveId];
  const art = EIN_ARTICLES.find((a) => a.id === einArticleId);
  if (!stem || !art) return null;
  const ending = art.ending;
  const form = applyEnding(stem, ending);
  const head = ending ? form.slice(0, form.length - ending.length) : form;
  const contracted = stem.toLowerCase() === "euer" && !!ending;
  ...
  const enWord = STEM_EN_SHORT[stem] ?? "my";
  return {
    stem: head,
    stemEn: STEM_EN[stem] ?? "",
    stemPerson: STEM_PERSON[stem] ?? "",
    ending,
    form,
    contracted,
    caseKey,
