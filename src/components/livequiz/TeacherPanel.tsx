...
const VERB_CTX: Record<string, { de: string; en: string; enPrep?: string }> = {
  ...
  gegen:     { de: "Er kämpft",      en: "He competes"      },
  ohne:      { de: "Er kommt",       en: "He comes"         },
  um:        { de: "Es geht",        en: "It's",  enPrep: "about" },
  ...
  nach:      { de: "Er fragt",       en: "He asks", enPrep: "about" },
...
function buildSentence(q: any) {
  ...
  if (q.prep.case === "nom") {
    ...
    return { deBefore: "", hint, deAfter: ` ${prep}${sfxDe}.`, en: `${hint} ${verbEn}${sfxEn}.`, enBefore: "", enBlank: hint, enAfter: ` ${verbEn}${sfxEn}.` };
  }

  if (q.kind === "pronoun") {
    const hint = q.targetEn ?? "";
    return { deBefore: `${ctx.de} ${prep} `, hint, deAfter: ".", en: `${ctx.en} ${prepEn} ${hint}.`, enBefore: `${ctx.en} ${prepEn} `, enBlank: hint, enAfter: "." };
  }
...
import { createLiveSession, getTeacherSession, updateLiveSession, listResponses, resetLiveSession } from "@/lib/livequiz.functions";
...
type Session = {
  id: string; code: string; host_token: string;
  phase: "lobby" | "active" | "results" | "ended";
  current_question_index: number; question_started_at: string | null;
  timer_max_seconds: number; questions: any[];
};
...
export function TeacherPanel() {
  ...
  async function createSession() {
    ...
    setCreating(true);
    try {
      const row = await createFn({ data: { code: makeCode(), gameMode: "prep-lock", questions: sampleQuestions(20), timerMaxSeconds: 30 } });
      setSession(row as Session);
      try { localStorage.setItem(STORAGE_KEY, JSON.stringify({ id: (row as any).id, hostToken: (row as any).host_token })); } catch {}
    } catch (e: any) {
  ...
  async function startQuiz() { await applyPatch({ phase: "active", current_question_index: 0, question_started_at: new Date().toISOString() }); }
  async function nextQuestion() {
    if (!session) return;
    const next = session.current_question_index + 1;
    if (next >= session.questions.length) { await applyPatch({ phase: "results" }); }
    else { await applyPatch({ current_question_index: next, question_started_at: new Date().toISOString() }); }
  }
  ...
  async function resetSession() {
    ...
    try {
      const row = await resetFn({ data: { sessionId: session.id, hostToken: session.host_token, questions: sampleQuestions(20) } });
      setSession(row as Session); setResponses([]);
    } catch (e: any) { alert(e?.message ?? "Reset failed"); }
  }

  const joinUrl = `${window.location.origin}/?student`;
  ...
  const activeQ = session?.phase === "active" ? session.questions?.[session.current_question_index] : null;
  ...
  useEffect(() => {
    const clear = () => document.querySelectorAll("[data-quiz-elim]").forEach((el) => el.removeAttribute("data-quiz-elim"));
    clear();
    ...
    activeTiers.order.slice(0, cappedElimCount).forEach((pillId) => {
      document.querySelectorAll(`[data-cell-id="${pillId}"]`).forEach((el) => (el as HTMLElement).setAttribute("data-quiz-elim", "1"));
    });
    return clear;
  }, [cappedElimCount, activeQ?.correctPillId, session?.phase]); // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => {
    const clear = () => document.querySelectorAll("[data-quiz-correct]").forEach((el) => el.removeAttribute("data-quiz-correct"));
    clear();
    if (!showBreakdown || !activeQ) return clear;
    document.querySelectorAll(`[data-cell-id="${activeQ.correctPillId}"]`).forEach((el) => (el as HTMLElement).setAttribute("data-quiz-correct", "1"));
    return clear;
  ...
  const answeredThisQ = session ? responses.filter((r) => r.question_index === session.current_question_index).length : 0;
  ...
  useEffect(() => {
    ...
    setShowBreakdown(false); setPanelExpanded(false); setStatsExpanded(false); setPillAvatars([]);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      for (const r of cur) { if (!byPill.has(r.answer)) byPill.set(r.answer, []); byPill.get(r.answer)!.push(r.student_avatar); }
      ...
      for (const [pillId, keys] of byPill) {
        ...
        keys.forEach((avatarKey, i) => { avatars.push({ key: `${pillId}-${i}`, x: cx + i * spacing, y: cy, avatarKey, delayMs: gi * 55, floatDur: 1.8 + Math.random() * 1.6, floatPhase: Math.random() * 2400 }); gi++; });
      }
  ...
  return (
    <>
      <div className="fixed top-9 left-4 z-[150] flex flex-col items-start gap-1.5 transition-transform duration-200 origin-top-left hover:scale-[2]">
        ...
        <div className="text-[10px] uppercase tracking-widest text-poster-ink/50 font-semibold">Scan to join!</div>
      </div>
      ...
      <div className="fixed left-0 top-[52px] bottom-0 w-[460px] flex flex-col p-4 gap-3 pointer-events-none"
        ...
        <div className="flex flex-col items-center justify-center text-center space-y-4 px-2 overflow-hidden"
          style={{ flexGrow: panelExpanded ? 0 : 1, opacity: panelExpanded ? 0 : 1, maxHeight: panelExpanded ? 0 : undefined, transition: "flex-grow 0.5s ease-in-out, opacity 0.35s ease-in-out, max-height 0.5s ease-in-out" }}>
          {activeQ && sentence && (
            <>
              ...
              <div className="text-5xl font-bold text-poster-ink leading-snug tracking-tight drop-shadow-sm">
                ...
                {showBreakdown ? (
                  <span key="filled" className="answer-fill text-poster-teal px-2">{PILL_LABEL[activeQ.correctPillId] ?? activeQ.correctPillId}</span>
                ) : (
                  <span key="blank" className="text-poster-ink/35 italic px-2 inline-block min-w-[64px] text-center" style={{ borderBottom: "3px solid black" }}>{sentence.hint || " "}</span>
                )}
        ...
        <div className="pointer-events-auto bg-poster-bg/95 backdrop-blur-sm rounded-2xl shadow-2xl overflow-hidden border border-poster-ink/10 flex flex-col"
          style={showBreakdown && !panelExpanded ? { maxHeight: 0, overflow: "hidden" } : { flexGrow: panelExpanded ? 1 : 0, transition: "flex-grow 0.5s ease-in-out" }}>
          <button onClick={() => setCollapsed((c) => !c)} className="w-full flex items-center justify-between px-4 py-3 bg-poster-teal text-white">
            ...
            <div className={cn("p-4 space-y-3 overflow-y-auto", panelExpanded ? "flex-1" : "max-h-[40vh]")}>
              {!session ? (
                ...
                <>
                      ...
                      {sessionLeaderboard.slice(0, 5).map((t, i) => {
                        ...
                        return (
                          <div key={t.id} className={cn("flex items-center gap-2 rounded-full px-3 py-1.5", i === 0 ? "bg-poster-yellow" : "bg-white/60")}
                            ...
                            {statsExpanded && (prevRank - i) !== 0 && prevRank !== -1 && (
                              <span className={cn("text-xs font-bold tabular-nums", (prevRank - i) > 0 ? "text-poster-teal" : "text-poster-red")}>
                                {(prevRank - i) > 0 ? `+${prevRank - i}` : prevRank - i}
                              </span>
                            )}
                  ...
                  <button onClick={() => { nextQuestion(); setShowBreakdown(false); }}
                  ...
                  <button onClick={resetSession} className="w-full py-2 text-xs text-poster-ink/30 hover:text-poster-ink/60 flex items-center justify-center gap-1.5 transition-colors">
                ...
                <>
                  <div>
                    <div className="text-[10px] uppercase tracking-widest text-poster-ink/40 font-semibold mb-2">Joined ({participants.size})</div>
                    <div className="flex flex-wrap gap-1.5">
                      {[...participants.values()].map((p, i) => (
                        <div key={i} className="flex items-center gap-1 bg-poster-yellow/20 rounded-full px-2 py-1 text-xs font-medium text-poster-ink">
                          <img src={avatarSrc(p.avatar)} alt="" className="w-4 h-4" />
                          <span>{p.name}</span>
                        </div>
                      ))}
                      {participants.size === 0 && <div className="text-xs text-poster-ink/30 italic">No students yet</div>}
                    </div>
                  </div>
                  {session.phase === "lobby" && (
                    <button onClick={startQuiz} disabled={participants.size === 0}
                  ...
                  {session.phase === "active" && (
                    <div className="space-y-2">
                      <div className="text-xs text-poster-ink/40 text-center">{answeredThisQ} / {participants.size} answered</div>
      ...
      {pillAvatars.map(({ key, x, y, avatarKey, delayMs, floatDur, floatPhase }) => (
        <div key={key} className="fixed z-[300] pointer-events-none avatar-scatter"
